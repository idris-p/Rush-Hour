...
const EXCLUDED_WALK_LINKS = new Set(["liverpool-street:moorgate"]);
...
const STATION_POSITION_OVERRIDES = new Map([
  ...
  ["pinner", { x: -24, y: -62 }],
  ["northwood-hills", { x: -28, y: -66 }],
  ["northwood", { x: -32, y: -70 }],
  ["moor-park", { x: -36, y: -74 }],
  ["rickmansworth", { x: -42, y: -80 }],
  ["chorleywood", { x: -46, y: -84 }],
  ["chalfont-and-latimer", { x: -50, y: -88 }],
  ["amersham", { x: -58, y: -88 }],
  ["chesham", { x: -54, y: -92 }],
  ["croxley", { x: -36, y: -82 }],
  ["watford", { x: -36, y: -86 }],
  ["canning-town", { x: 154, y: 4 }],
  ["canada-water", { x: 118, y: 16 }],
  ["battersea-power-station", { x: 30, y: 38 }],
  ["nine-elms", { x: 38, y: 38 }],
  ["gunnersbury", { x: -26, y: 20 }],
  ["kew-gardens", { x: -32, y: 26 }],
  ["richmond", { x: -38, y: 32 }],
  ["lambeth-north", { x: 64, y: 30 }],
  ["embankment", { x: 62, y: 14 }],
]);
...
for (const station of stationByGroup.values()) {
  const override = STATION_POSITION_OVERRIDES.get(station.id);
  if (override) Object.assign(station, override);
}
...
applyConnectionPathOverrides(connections);
...
function addWalkConnection(connectionById, fromStation, toStation, sourcePath = null) {
  ...
  const [a, b] = [fromStation.id, toStation.id].sort();
  if (EXCLUDED_WALK_LINKS.has(`${a}:${b}`)) return;
  const id = `walk:${a}:${b}`;
...
function applyConnectionPathOverrides(connections) {
  const overrides = new Map([
    [
      "northern:waterloo:kennington",
      expandGridPath([
        { x: 62, y: 20 },
        { x: 60, y: 22 },
        { x: 60, y: 38 },
  ...
  for (const connection of connections) {
    const path = overrides.get(connectionKey(connection));
    if (path) connection.path = path;
  }
}
